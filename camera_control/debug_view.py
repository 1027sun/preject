...
# MediaPipe Pose landmarks. These remain private to the debug view.
_EDGES = ((11, 12), (11, 13), (13, 15), (12, 14), (14, 16),
        ...
        if frame is None:
            ...
            if landmarks:
                ...
                for first, second in _EDGES:
                    if max(first, second) >= len(landmarks):
                        continue
                    if min(getattr(landmarks[first], "visibility", 1.0),
                           getattr(landmarks[second], "visibility", 1.0)) >= 0.5:
                        cv2.line(frame, point(first), point(second), (170, 218, 78), 3, cv2.LINE_AA)
                for index in {item for edge in _EDGES for item in edge}:
                    if index < len(landmarks) and getattr(landmarks[index], "visibility", 1.0) >= 0.5:
                        cv2.circle(frame, point(index), 4, (255, 245, 225), -1, cv2.LINE_AA)
        height, width = frame.shape[:2]
