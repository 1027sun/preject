...
小窗里要同时看到骨骼与关键点，用 `get_annotated_preview(width=320)`：返回同样的镜像 RGB 小图，但已按最近一次姿态结果画好骨架。没有检测到人体（漏检、未校准或结果过期）时不画骨架，返回的就是普通画面，因此不会停留在旧骨架上。该接口同样不消费跳跃事件，也不会修改 `get_preview_frame()` 的内容。

```python
frame = controller.get_annotated_preview(width=320)   # 带骨骼；无姿态时等同 get_preview_frame
```

骨架拓扑与可见度规则集中在 `camera_control/pose_view.py`（`POSE_EDGES` 与 `draw_pose`），独立调试窗口和游戏预览共用同一份实现；需要更细的手动绘制时可以直接复用 `draw_pose(frame_bgr, landmarks, scale=...)`。
